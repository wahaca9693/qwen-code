...
1. Extract components from `vscode-ide-companion/src/webview/` into an independent `@zero-agent/webui` package
...
```typescript
// @zero-agent/webui/src/context/PlatformContext.ts
...
```javascript
// @zero-agent/webui/tailwind.preset.js
module.exports = {
  theme: {
    extend: {
      colors: {
...
module.exports = {
  presets: [require('@zero-agent/webui/tailwind.preset')],
  content: [
    './src/**/*.{ts,tsx}',
    './node_modules/@zero-agent/webui/dist/**/*.js',
  ],
};
```
...
```
Developer ──► @zero-agent/webui ──► vscode-ide-companion
  │              │                      │
...
// After: Update import
import { FileIcon } from '@zero-agent/webui';
// or import { FileIcon } from '@zero-agent/webui/icons';
```
...
1. **Tailwind Class Name Tree Shaking**
   ...
   - Solution: Consumer's `content` config needs to include `node_modules/@zero-agent/webui`
...
│     └── Edit files in @zero-agent/webui/src/                     │
│                                                                  │
...
│     └── import { Component } from '@zero-agent/webui'            │
...
1. **Single Source of Truth**: All UI components live in `@zero-agent/webui`
