      ...
      for (const reqInfo of requestsToProcess) {
        ...
        if (pm && !(await pm.isToolEnabled(reqInfo.name))) {
          ...
          const ruleInfo = matchingRule
            ...
            : '';
          const permissionErrorMessage = `ZERO Agent requires permission to use "${reqInfo.name}", but that permission was declined.${ruleInfo}`;
          newToolCalls.push({
            status: 'error',
            request: reqInfo,
        ...
        if (!pm) {
          ...
          if (excludeTools && excludeTools.length > 0) {
            ...
            if (excludedMatch) {
              const permissionErrorMessage = `ZERO Agent requires permission to use ${excludedMatch}, but that permission was declined.`;
              newToolCalls.push({
                status: 'error',
                request: reqInfo,
                response: createErrorResponse(
        ...
        try {
          ...
          if (!needsConfirmation(finalPermission, approvalMode, reqInfo.name)) {
            ...
            if (isNonInteractiveDeny) {
              const errorMessage = `ZERO Agent requires permission to use "${reqInfo.name}", but that permission was declined (non-interactive mode cannot prompt for confirmation).`;
              this.setStatusInternal(
                reqInfo.callId,
                'error',
                createErrorResponse(
            ...
            if (hooksEnabled && messageBus) {
              fireNotificationHook(
                messageBus,
                `ZERO Agent needs your permission to use ${reqInfo.name}`,
