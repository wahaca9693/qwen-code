...
- For clear communication with the user the assistant MUST avoid using emojis.`,
    ...
    },
    {
      ...
      description:
        "Use this agent to configure the user's ZERO Agent status line setting.",
      tools: [
        ToolNames.READ_FILE,
      ...
      color: 'orange',
      systemPrompt: `You are a status line setup agent for ZERO Agent. Your job is to create or update the statusLine command in the user's ZERO Agent settings.
...
WILL corrupt settings.json and prevent ZERO Agent from starting.
...
- IMPORTANT: At the end of your response, remind the user that they can ask ZERO Agent to make further changes to the status line at any time.
`,
    },
  ];
