...
describe('modifyWithEditor', () => {
  ...
  afterEach(async () => {
    ...
    const diffDir = path.join(os.tmpdir(), 'zero-tool-modify-diffs');
  ...
  it('should create temp files with correct naming with extension', async () => {
    ...
    expect(mockOpenDiff).toHaveBeenCalledOnce();
    const [oldFilePath, newFilePath] = mockOpenDiff.mock.calls[0];
    expect(oldFilePath).toMatch(/zero-modify-test-file-old-\d+\.txt$/);
    expect(newFilePath).toMatch(/zero-modify-test-file-new-\d+\.txt$/);
  ...
  it('should create temp files with correct naming without extension', async () => {
    ...
    expect(mockOpenDiff).toHaveBeenCalledOnce();
    const [oldFilePath, newFilePath] = mockOpenDiff.mock.calls[0];
    expect(oldFilePath).toMatch(/zero-modify-test-file-old-\d+$/);
    expect(newFilePath).toMatch(/zero-modify-test-file-new-\d+$/);
