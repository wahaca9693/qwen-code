...
import { isGitRepository } from '@zero-agent/zero-core';
...
describe('getInstallationInfo', () => {
  ...
  it('should detect global pnpm installation', () => {
    const pnpmPath = `/Users/test/.pnpm/global/5/node_modules/.pnpm/some-hash/node_modules/@zero/zero/dist/index.js`;
    ...
    expect(info.isGlobal).toBe(true);
    expect(info.updateCommand).toBe('pnpm add -g @zero/zero@latest');
  ...
  it('should detect global yarn installation', () => {
    const yarnPath = `/Users/test/.yarn/global/node_modules/@zero/zero/dist/index.js`;
    ...
    expect(info.updateCommand).toBe(
      'yarn global add @zero/zero@latest',
    );
    expect(info.updateMessage).toContain('Attempting to automatically update');
  ...
  it('should detect global bun installation', () => {
    ...
    expect(info.isGlobal).toBe(true);
    expect(info.updateCommand).toBe('bun add -g @zero/zero@latest');
    expect(info.updateMessage).toContain('Attempting to automatically update');
  ...
  it('should default to global npm installation for unrecognized paths', () => {
    ...
    expect(info.isGlobal).toBe(true);
    expect(info.updateCommand).toBe(
      'npm install -g @zero/zero@latest',
    );
    expect(info.updateMessage).toContain('Attempting to automatically update');
