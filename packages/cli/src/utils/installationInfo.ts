...
import { createDebugLogger, isGitRepository } from '@zero-agent/zero-core';
  ...
  try {
    ...
    if (process.platform === 'darwin') {
      try {
        ...
        childProcess.execSync('brew list -1 | grep -q "^zero$"', {
          stdio: 'ignore',
        });
        return {
    ...
    if (realPath.includes('/.pnpm/global')) {
      const updateCommand = 'pnpm add -g @zero/zero@latest';
      return {
    ...
    if (realPath.includes('/.yarn/global')) {
      const updateCommand = 'yarn global add @zero/zero@latest';
      return {
    ...
    if (realPath.includes('/.bun/bin')) {
      const updateCommand = 'bun add -g @zero/zero@latest';
      return {
    ...
    // Assume global npm
    const updateCommand = 'npm install -g @zero/zero@latest';
    return {
