...
export const buildRemovalNotice = (): string =>
  [
    '',
    yellow(t('⚠  ZERO auth has been removed.')),
    '',
    `  ${cyan(t('Interactive'))}   →  ${t('run ZERO and use /auth to configure providers')}`,
    ...
    `  ${cyan(t('Qwen OAuth'))}    →  ${t('run ZERO interactively and use /auth; OAuth cannot be configured with env vars alone')}`,
    `  ${cyan(t('Scripted'))}      →  ${t('edit ~/.qwen/settings.json, or run ZERO interactively once')}`,
    '',
