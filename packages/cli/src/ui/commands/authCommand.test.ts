  ...
  it.each(['non_interactive', 'acp'] as const)(
    ...
    (executionMode) => {
      ...
      expect(result).toEqual({
        ...
        messageType: 'info',
        content:
          'Authentication configuration is only available in interactive mode. To configure authentication, run ZERO Agent interactively and use /auth, or set environment variables: OPENAI_API_KEY, OPENAI_BASE_URL, OPENAI_MODEL.',
      });
    },
  );
