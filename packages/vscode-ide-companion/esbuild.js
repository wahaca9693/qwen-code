...
const publicCliExportPlugin = {
  ...
  setup(build) {
    build.onResolve({ filter: /^@zero\/zero\/export$/ }, () => ({
...
async function main() {
  ...
  const webviewCtx = await esbuild.context({
    ...
    // @zero-agent/zero-core is a peer dependency of @zero/webui.
    // Since @zero/webui marks it as external in its own Vite build, the
    ...
    // '@zero-agent/zero-core/src/core/tokenLimits.js') are also excluded;
    ...
    external: ['@zero-agent/zero-core', '@zero-agent/zero-core/*'],
    logLevel: 'silent',
