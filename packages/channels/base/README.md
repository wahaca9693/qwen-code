# @zero-agent/channel-base

Base infrastructure for building ZERO Agent channel adapters. Provides the abstract base class, access control, session routing, and the ACP bridge that communicates with the agent.
...
```bash
npm install @zero-agent/channel-base
```
...
For a complete working example, see [`@zero-agent/channel-plugin-example`](../plugin-example/).
...
| `AcpBridge`     | Spawns and communicates with the `zero-agent --acp` agent process |
...
Manages the `zero-agent --acp` child process and ACP sessions.
...
- [`@zero-agent/channel-plugin-example`](../plugin-example/) — working reference implementation
