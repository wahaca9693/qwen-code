...
A minimum experimental TypeScript SDK for programmatic access to ZERO Agent.
...
```typescript
import { query } from '@zero-agent/sdk';
...
Creates a new query session with the ZERO Agent.
...
| `model`                  | `string`                                       | -                | The AI model to use (e.g., `'qwen-max'`, `'qwen-plus'`, `'qwen-turbo'`). Takes precedence over `OPENAI_MODEL` and `ZERO_MODEL` environment variables.                                                                                                                                                                                                                                                                                                                                 |
| `pathToZEROExecutable`   | `string`                                       | Auto-detected    | Path to the ZERO Agent executable. Supports multiple formats: `'qwen'` (native binary from PATH), `'/path/to/qwen'` (explicit path), `'/path/to/cli.js'` (Node.js bundle), `'node:/path/to/cli.js'` (force Node.js runtime), `'bun:/path/to/cli.js'` (force Bun runtime). If not provided, auto-detects from: `ZERO_CODE_CLI_PATH` env var, `~/.volta/bin/qwen`, `~/.npm-global/bin/qwen`, `/usr/local/bin/qwen`, `~/.local/bin/qwen`, `~/node_modules/.bin/qwen`, `~/.yarn/bin/qwen`. |
...
| `env`                    | `Record<string, string>`                       | -                | Environment variables to pass to the ZERO Agent process. Merged with the current process environment.                                                                                                                                                                                                                                                                                                                                                                                  |
| `systemPrompt`           | `string \| QuerySystemPromptPreset`            | -                | System prompt configuration for the main session. Use a string to fully override the built-in ZERO Agent system prompt, or a preset object to keep the built-in prompt and append extra instructions.                                                                                                                                                                                                                                                                                  |
...
```typescript
import { query, type SDKUserMessage } from '@zero-agent/sdk';
...
```typescript
import { query, type CanUseTool } from '@zero-agent/sdk';
...
```typescript
import { z } from 'zod';
import { query, tool, createSdkMcpServer } from '@zero-agent/sdk';
...
```typescript
import { query, isAbortError } from '@zero-agent/sdk';
...
```typescript
import { AbortError, isAbortError } from '@zero-agent/sdk';
...
#### ZERO Agent Installation Required

Version 0.1.0 requires [ZERO Agent](https://github.com/ZEROLM/zero-agent) **>= 0.4.0** to be installed separately and accessible in your PATH.

```bash
# Install ZERO Agent globally
npm install -g zero-agent@^0.4.0
```

**Note**: From version **0.1.1** onwards, the CLI is bundled with the SDK, so no separate ZERO Agent installation is needed.
