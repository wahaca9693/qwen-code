...
function findSdkPackageRoot(): string | null {
  try {
    ...
    const packageJsonPath = require.resolve('@zero-agent/sdk/package.json');
    ...
    if (fs.existsSync(packageJsonPath)) {
      ...
      if (fs.existsSync(cliPath)) {
        try {
          ...
          if (packageJson.name === '@zero-agent/sdk') {
            return dir;
          }
          if (!bestMatch) {
            bestMatch = dir;
...
export function findBundledCliPath(): string {
  ...
  const candidates = getBundledCliCandidatePaths();
  throw new Error(
    'Bundled ZERO CLI not found. The CLI should be included in the SDK package.\n' +
      'Searched locations:\n' +
