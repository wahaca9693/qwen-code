# MCP servers with ZERO Agent

This document provides a guide to configuring and using Model Context Protocol (MCP) servers with ZERO Agent.
...
ZERO Agent integrates with MCP servers through a sophisticated discovery and execution system built into the core package (`packages/core/src/tools/`):
...
4. **Sanitizes and validates** tool schemas for compatibility with the ZERO API
...
ZERO Agent uses the `mcpServers` configuration in your `settings.json` file to locate and connect to MCP servers. This configuration supports multiple servers with different transport mechanisms.
...
ZERO Agent supports OAuth 2.0 authentication for remote MCP servers using SSE or HTTP transports. This enables secure access to MCP servers that require authentication.
...
```bash
zero mcp add --transport sse remote-server https://api.example.com/sse/ \
...
- **`redirectUri`** (string): Custom redirect URI. **Critical for remote deployments**: Defaults to `http://localhost:7777/oauth/callback`. When running ZERO Agent on remote/cloud servers, set this to a publicly accessible URL (e.g., `https://your-server.com/oauth/callback`). Can be configured via `zero mcp add --oauth-redirect-uri` or directly in settings.json.
...
When ZERO Agent starts, it performs MCP server discovery through the following detailed process:
...
4. **Name sanitization:** Tool names are cleaned to meet ZERO API requirements:
...
Once discovered, MCP tools are available to the ZERO model like built-in tools. The model will automatically:
...
When ZERO Agent receives this response, it will:
...
This enables you to build sophisticated tools that can provide rich, multi-modal context to the ZERO model.

## MCP Prompts as Slash Commands

In addition to tools, MCP servers can expose predefined prompts that can be executed as slash commands within ZERO Agent. This allows you to create shortcuts for common or complex queries that can be easily invoked by name.
...
```bash
/poem-writer --title="ZERO Agent" --mood="reverent"
```

or, using positional arguments:

```bash
/poem-writer "ZERO Agent" reverent
```
...
## Managing MCP Servers with `zero mcp`
...
```bash
zero mcp add [options] <name> <commandOrUrl> [args...]
```
...
- `--oauth-redirect-uri`: OAuth redirect URI (e.g., `https://your-server.com/oauth/callback`). Defaults to `http://localhost:7777/oauth/callback` for local setups. **Important for remote deployments**: When running ZERO Agent on remote/cloud servers, set this to a publicly accessible URL.
...
```bash
# Basic syntax
zero mcp add <name> <command> [args...]

# Example: Adding a local server
zero mcp add my-stdio-server -e API_KEY=123 /path/to/server arg1 arg2 arg3

# Example: Adding a local python server
zero mcp add python-server python server.py --port 8080
```
...
```bash
# Basic syntax
zero mcp add --transport http <name> <url>

# Example: Adding an HTTP server
zero mcp add --transport http http-server https://api.example.com/mcp/

# Example: Adding an HTTP server with an authentication header
zero mcp add --transport http secure-http https://api.example.com/mcp/ --header "Authorization: Bearer abc123"
```
...
```bash
# Basic syntax
zero mcp add --transport sse <name> <url>

# Example: Adding an SSE server
zero mcp add --transport sse sse-server https://api.example.com/sse/

# Example: Adding an SSE server with an authentication header
zero mcp add --transport sse secure-sse https://api.example.com/sse/ --header "Authorization: Bearer abc123"

# Example: Adding an OAuth-enabled SSE server
zero mcp add --transport sse oauth-server https://api.example.com/sse/ \
...
### Managing Servers (`zero mcp`)

To view and manage all MCP servers currently configured, use the `manage` command or simply `zero mcp`. This opens an interactive TUI dialog where you can:
...
### Removing a Server (`zero mcp remove`)
...
```bash
zero mcp remove <name>
```

**Example:**

```bash
zero mcp remove my-server
```
