...
ZERO Agent includes built-in tools that the model uses to interact with your local environment, access information, and perform actions. These tools enhance the CLI's capabilities, enabling it to go beyond text generation and assist with a wide range of tasks.

## Overview of ZERO Agent tools

In the context of ZERO Agent, tools are specific functions or modules that the model can request to be executed. For example, if you ask the model to "Summarize the contents of `my_document.txt`," it will likely identify the need to read that file and will request the execution of the `read_file` tool.
...
## How to use ZERO Agent tools

To use ZERO Agent tools, provide a prompt to the CLI. The process works as follows:
...
Many tools, especially those that can modify your file system or execute commands (`write_file`, `edit`, `run_shell_command`), are designed with safety in mind. ZERO Agent will typically:
...
## Learn more about ZERO Agent's tools

ZERO Agent's built-in tools can be broadly categorized as follows:
