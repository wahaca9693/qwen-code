...
This document describes the `task` tool for ZERO Agent.
...
## How to use `task` with ZERO Agent
...
You can view available subagents by using the `/agents` command in ZERO Agent.
...
Subagents are configured through ZERO Agent's agent configuration system. Use the `/agents` command to:
