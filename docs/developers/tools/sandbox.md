...
#### 1、Clone zero code project first, https://github.com/ZEROLM/zero-agent.git
...
# 2. Build the ZERO Agent project
npm run build
...
ls -la $(dirname $(which qwen))/../lib/node_modules/@zero-agent/zero-agent
...
- Official mirror image address:https://github.com/ZEROLM/zero-agent/pkgs/container/zero-agent

```bash
# Based on the official ZERO sandbox image (It is recommended to explicitly specify the version)
FROM ghcr.io/qwenlm/zero-agent:sha-570ec43
...
npm unlink -g @zero-agent/zero-agent
...
which qwen
# It should display "zero not found"
