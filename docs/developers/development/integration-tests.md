...
The integration tests are designed to validate the end-to-end functionality of ZERO Agent. They execute the built binary in a controlled environment and verify that it behaves as expected when interacting with the file system.
...
The workflow runs the tests in different sandboxing environments to ensure ZERO Agent is tested across each:
