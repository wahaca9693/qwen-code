...
This monorepo contains two main packages: `@zero-agent/zero-agent` and `@zero-agent/zero-agent-core`.

## `@zero-agent/zero-agent`

This is the main package for ZERO Agent. It is responsible for the user interface, command parsing, and all other user-facing functionality.

When this package is published, it is bundled into a single executable file. This bundle includes all of the package's dependencies, including `@zero-agent/zero-agent-core`. This means that whether a user installs the package with `npm install -g @zero-agent/zero-agent` or runs it directly with `npx @zero-agent/zero-agent`, they are using this single, self-contained executable.

## `@zero-agent/zero-agent-core`
...
Releases are managed through the [release.yml](https://github.com/ZEROLM/zero-agent/actions/workflows/release.yml) GitHub Actions workflow. To perform a manual release for a patch or hotfix:
...
# Preview
npm install -g @zero-agent/zero-agent@preview

# Nightly
npm install -g @zero-agent/zero-agent@nightly
```
...
- `npx -y @zero-agent/zero-agent@latest --version` to validate the push worked as expected if you were not doing a rc or dev tag
- `npx -y @zero-agent/zero-agent@<release tag> --version` to validate the tag pushed appropriately
- _This is destructive locally_ `npm uninstall @zero-agent/zero-agent && npm uninstall -g @zero-agent/zero-agent && npm cache clean --force &&  npm install @zero-agent/zero-agent@<version>`
...
1.  Go to the [Actions tab](https://github.com/ZEROLM/zero-agent/actions/workflows/release.yml) of the repository.
...
You can then inspect the generated tarballs to ensure that they contain the correct files and that the `package.json` files have been updated correctly. The tarballs will be created in the root of each package's directory (e.g., `packages/cli/zero-agent-0.1.6.tgz`).
...
- **Simplified Script Execution**: You can run scripts in any package from the root of the project using the `--workspace` flag. For example, to run the `build` script in the `cli` package, you can run `npm run build --workspace @zero-agent/zero-agent`.
