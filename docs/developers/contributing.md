...
```bash
git clone https://github.com/ZEROLM/zero-agent.git # Or your fork's URL
cd zero-agent
```
...
[Sandboxing](#sandboxing) is highly recommended and requires, at a minimum, setting `ZERO_SANDBOX=true` in your `~/.env` and ensuring a sandboxing provider (e.g. `macOS Seatbelt`, `docker`, or `podman`) is available. See [Sandboxing](#sandboxing) for details.

To build both the `zero-agent` CLI utility and the sandbox container, run `build:all` from the root directory:
...
To start the ZERO Agent application from the source code (after building), run the following command from the root directory:

```bash
npm start
```

If you'd like to run the source build outside of the zero-agent folder, you can utilize `npm link path/to/zero-agent/packages/cli` (see: [docs](https://docs.npmjs.com/cli/v9/commands/npm-link)) to run with `zero-agent`
...
The integration tests are designed to validate the end-to-end functionality of ZERO Agent. They are not run as part of the default `npm run test` command.
...
- `packages/`: Contains the individual sub-packages of the project.
  ...
  - `core/`: The core backend logic for ZERO Agent.
...
**Note:** If you have `DEBUG=true` in a project's `.env` file, it won't affect zero-agent due to automatic exclusion. Use `.zero-agent/.env` files for zero-agent specific debug settings.

### React DevTools

To debug the CLI's React-based UI, you can use React DevTools. Ink, the library used for the CLI's interface, is compatible with React DevTools version 4.x.

1.  **Start the ZERO Agent application in development mode:**
