...
session); disable via `ZERO_DEBUG_LOG_FILE=0`.
...
| `ZERO_CODE_ENABLE_AWAY_SUMMARY` env var          | Claude Code uses it to keep the feature on when telemetry is disabled; ZERO Agent's current telemetry model doesn't need this.            |
