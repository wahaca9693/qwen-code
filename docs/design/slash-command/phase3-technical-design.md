...
- `src/commands.ts`：Claude Code 内置 `/doctor`、`/release-notes` 等命令，ZERO Agent 当前已实现 `/doctor`；本阶段不实现 `/release-notes`。
...
tmux new-session -d -s qwen-slash-phase3 -x 200 -y 50 \
  "cd /tmp/qwen-slash-phase3 && /Users/mochi/code/zero-agent-test/dist/cli.js --approval-mode yolo"
sleep 3
```
