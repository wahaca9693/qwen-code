...
This is the most flexible and simple option. All you need to do us create a publicly accessible git repo (such as a public github repository) and then users can install your extension using `zero extensions install <your-repo-uri>`, or for a GitHub repository they can use the simplified `zero extensions install <org>/<repo>` format. They can optionally depend on a specific ref (branch/tag/commit) using the `--ref=<some-ref>` argument, this defaults to the default branch.
...
While there are many options for how you want to manage releases using a git flow, we recommend treating your default branch as your "stable" release branch. This means that the default behavior for `zero extensions install <your-repo-uri>` is to be on the stable release branch.
...
ZERO Agent extensions can be distributed through [GitHub Releases](https://docs.github.com/en/repositories/releasing-projects-on-github/about-releases). This provides a faster and more reliable initial installation experience for users, as it avoids the need to clone the repository.
...
When checking for updates, zero code will just look for the latest release on github (you must mark it as such when creating the release), unless the user installed a specific release by passing `--ref=<some-release-tag>`. We do not at this time support opting in to pre-release releases or semver.
...
To ensure ZERO Agent can automatically find the correct release asset for each platform, you must follow this naming convention. The CLI will search for assets in the following order:
...
Here is an example of a GitHub Actions workflow that builds and releases a ZERO Agent extension for multiple platforms:
...
You can publish ZERO Agent extensions as scoped npm packages (e.g. `@your-org/my-extension`). This is a good fit when:
...
Your npm package must include a `qwen-extension.json` file at the package root. This is the same config file used by all ZERO Agent extensions — the npm tarball is simply another delivery mechanism.
...
zero extensions install @your-org/my-extension

# Install a specific version
zero extensions install @your-org/my-extension@1.2.0

# Install from a custom registry
zero extensions install @your-org/my-extension --registry https://your-registry.com
```
...
ZERO Agent reads npm auth credentials automatically:
...
zero extensions install @your-org/my-extension@beta
```
