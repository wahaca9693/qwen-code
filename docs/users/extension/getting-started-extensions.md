# Getting Started with ZERO Agent Extensions

This guide will walk you through creating your first ZERO Agent extension. You'll learn how to set up a new extension, add a custom tool via an MCP server, create a custom command, and provide context to the model with a `QWEN.md` file.

## Prerequisites

Before you start, make sure you have the ZERO Agent installed and a basic understanding of Node.js and TypeScript.
...
```bash
zero extensions new my-first-extension mcp-server
```
...
This is the manifest file for your extension. It tells ZERO Agent how to load and use your extension.
...
- `mcpServers`: This section defines one or more Model Context Protocol (MCP) servers. MCP servers are how you can add new tools for the model to use.
  - `command`, `args`, `cwd`: These fields specify how to start your server. Notice the use of the `${extensionPath}` variable, which ZERO Agent replaces with the absolute path to your extension's installation directory. This allows your extension to work regardless of where it's installed.
...
Before you can use the extension, you need to compile the TypeScript code and link the extension to your ZERO Agent installation for local development.
...
3.  **Link the extension:**
    ...
    The `link` command creates a symbolic link from the ZERO Agent extensions directory to your development directory. This means any changes you make will be reflected immediately without needing to reinstall.

    ```bash
    zero extensions link .
    ```

Now, restart your ZERO Agent session. The new `fetch_posts` tool will be available. You can test it by asking: "fetch posts".
...
After saving the file, restart the ZERO Agent. You can now run `/fs:grep-code "some pattern"` to use your new command.

## Step 5: Add Custom Skills and Subagents (Optional)

Extensions can also provide custom skills and subagents to extend ZERO Agent's capabilities.
...
After restarting ZERO Agent, your custom skills will be available via `/skills` and subagents via `/agents manage`.
...
You've successfully created a ZERO Agent extension! You learned how to:
...
From here, you can explore more advanced features and build powerful new capabilities into the ZERO Agent.
