...
- **Error: `ZERO OAuth free tier was discontinued on 2026-04-15`**
  - **Cause:** ZERO OAuth is no longer available as of April 15, 2026.
...
- **Error: `Device authorization flow failed: fetch failed`**
  - **Cause:** Node.js could not reach ZERO OAuth endpoints (often a proxy or SSL/TLS trust issue). When available, ZERO Agent will also print the underlying error cause (for example: `UNABLE_TO_VERIFY_LEAF_SIGNATURE`). Note: this error is specific to the legacy ZERO OAuth flow.
  - **Solution:**
    - If you are still using ZERO OAuth, switch to API Key or Coding Plan via `/auth`.
    - If you are behind a proxy, set it via `zero --proxy <url>` (or the `proxy` setting in `settings.json`).
...
- **Q: How do I update ZERO Agent to the latest version?**
  - A: If you installed it globally via `npm`, update it using the command `npm install -g @zero-agent/zero-agent@latest`. If you compiled it from source, pull the latest changes from the repository, and then rebuild using the command `npm run build`.

- **Q: Where are the ZERO Agent configuration or settings files stored?**
  - A: The ZERO Agent configuration is stored in two `settings.json` files:
    1. In your home directory: `~/.qwen/settings.json`.
    2. In your project's root directory: `./.qwen/settings.json`.

    Refer to [ZERO Agent Configuration](../configuration/settings) for more details.
...
- **Error: Command not found (when attempting to run ZERO Agent with `qwen`).**
  - **Cause:** The CLI is not correctly installed or it is not in your system's `PATH`.
  - **Solution:**
    The update depends on how you installed ZERO Agent:
    - If you installed `qwen` globally, check that your `npm` global binary directory is in your `PATH`. You can update using the command `npm install -g @zero-agent/zero-agent@latest`.
...
- **Error: "Operation not permitted", "Permission denied", or similar.**
  - **Cause:** When sandboxing is enabled, ZERO Agent may attempt operations that are restricted by your sandbox configuration, such as writing outside the project directory or system temp directory.
...
- **ZERO Agent is not running in interactive mode in "CI" environments**
  - **Issue:** ZERO Agent does not enter interactive mode (no prompt appears) if an environment variable starting with `CI_` (e.g. `CI_TOKEN`) is set. This is because the `is-in-ci` package, used by the underlying UI framework, detects these variables and assumes a non-interactive CI environment.
...
- Restart the integrated terminal after installing the extension so it inherits:
  - `ZERO_CODE_IDE_WORKSPACE_PATH`
  - `ZERO_CODE_IDE_SERVER_PORT`
- If running in a container, verify `host.docker.internal` resolves. Otherwise, map the host appropriately.
- Reinstall the companion with `/ide install` and use “ZERO Agent: Run” in the Command Palette to verify it launches.

## Exit Codes

The ZERO Agent uses specific exit codes to indicate the reason for termination. This is especially useful for scripting and automation.
...
If you encounter an issue that was not covered here in this _Troubleshooting guide_, consider searching the ZERO Agent [Issue tracker on GitHub](https://github.com/ZEROLM/zero-agent/issues). If you can't find an issue similar to yours, consider creating a new GitHub Issue with a detailed description. Pull requests are also welcome!
