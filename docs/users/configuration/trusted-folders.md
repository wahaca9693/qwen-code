...
The Trusted Folders feature is a security setting that gives you control over which projects can use the full capabilities of the ZERO Agent. It prevents potentially malicious code from running by asking you to approve a folder before the CLI loads any project-specific configurations from it.
...
Once the feature is enabled, the first time you run the ZERO Agent from a folder, a dialog will automatically appear, prompting you to make a choice:
...
When a folder is **untrusted**, the ZERO Agent runs in a restricted "safe mode" to protect you. In this mode, the following features are disabled:
...
Granting trust to a folder unlocks the full functionality of the ZERO Agent for that workspace.
