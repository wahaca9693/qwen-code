...
ZERO Agent supports three authentication methods. Pick the one that matches how you want to run the CLI:

- **ZERO OAuth**: sign in with your `qwen.ai` account in a browser. **Free tier discontinued on 2026-04-15** — switch to another method.
...
## Option 1: ZERO OAuth (Discontinued)

> [!warning]
>
> The ZERO OAuth free tier was discontinued on 2026-04-15. Existing cached tokens may continue working briefly, but new requests will be rejected. Please switch to Alibaba Cloud Coding Plan, [OpenRouter](https://openrouter.ai), [Fireworks AI](https://app.fireworks.ai), or another provider. Run `qwen` and use `/auth` to configure.

- **How it works**: on first start, ZERO Agent opens a browser login page. After you finish, credentials are cached locally so you usually won't need to log in again.
...
- **How it works**: Subscribe to the Coding Plan with a fixed monthly fee, then configure ZERO Agent to use the dedicated endpoint and your subscription API key.
...
- **Benefits**: Diverse model options, higher usage quotas, predictable monthly costs, access to a wide range of models (ZERO, GLM, Kimi, Minimax and more).
...
Enter `qwen` in the terminal to launch ZERO Agent, then run the `/auth` command and select **Alibaba Cloud Coding Plan**. Choose your region, then enter your `sk-sp-xxxxxxxxx` key.
...
For CI, containers, or scripts, configure Coding Plan with environment variables or `settings.json` instead of the removed `zero auth coding-plan` command.
...
| `security.auth.selectedType` | Tells ZERO Agent which protocol to use on startup (e.g. `openai`, `anthropic`, `gemini`). Without this, you'd need to run `/auth` interactively. |
| `model.name`                 | The default model to activate when ZERO Agent starts. Must match one of the `id` values in your `modelProviders`.                                |
...
The key concept is **Model Providers** (`modelProviders`): ZERO Agent supports multiple API protocols, not just OpenAI. You configure which providers and models are available by editing `~/.qwen/settings.json`, then switch between them at runtime with the `/model` command.
...
ZERO Agent reads API keys from environment variables (specified by `envKey` in your model config). There are multiple ways to provide them, listed below from **highest to lowest priority**:
...
ZERO Agent auto-loads the **first** `.env` file it finds (variables are **not merged** across multiple files). Only variables not already present in `process.env` are loaded.

Search order (from the current directory, walking upward toward `/`):

1. `.qwen/.env` (preferred — keeps ZERO Agent variables isolated from other tools)
2. `.env`
...
> [!tip]
>
> `.qwen/.env` is recommended over `.env` to avoid conflicts with other tools. Some variables (like `DEBUG` and `DEBUG_MODE`) are excluded from project-level `.env` files to avoid interfering with ZERO Agent behavior.
...
After launching ZERO Agent, use the `/model` command to switch between all configured models. Models are grouped by protocol:
...
zero --model "qwen3-coder-plus"

# In another terminal

zero --model "qwen3.5-plus"
```

## Removed `zero auth` CLI command

The standalone `zero auth` CLI command has been removed. Use these replacements instead:
...
| Check current authentication     | Run `/doctor` inside ZERO Agent                                                              |
| OAuth browser flow               | Run `qwen` interactively and use `/auth`; OAuth cannot be configured with env vars alone    |

Legacy invocations such as `zero auth status` now print a removal notice with these migration paths.
