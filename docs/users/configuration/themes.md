...
ZERO Agent supports a variety of themes to customize its color scheme and appearance. You can change the theme to suit your preferences via the `/theme` command or `"theme":` configuration setting.

## Available Themes

ZERO Agent comes with a selection of pre-defined themes, which you can list using the `/theme` command within the CLI:
...
Selected themes are saved in ZERO Agent's [configuration](../configuration/settings) so your preference is remembered across sessions.

---

## Custom Color Themes

ZERO Agent allows you to create your own custom color themes by specifying them in your `settings.json` file. This gives you full control over the color palette used in the CLI.
...
- Select your custom theme using the `/theme` command in ZERO Agent. Your custom theme will appear in the theme selection dialog.
