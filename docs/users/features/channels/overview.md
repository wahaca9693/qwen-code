...
Channels let you interact with a ZERO Agent agent from messaging platforms like Telegram, WeChat, or DingTalk, instead of the terminal. You send messages from your phone or desktop chat app, and the agent responds just like it would in the CLI.

## How It Works

When you run `zero channel start`, ZERO Agent:
...
3. Run `zero channel start` to start all channels, or `zero channel start <name>` for a single channel
...
```bash
zero channel pairing approve my-channel VEQDDWXJ
```
...
zero channel pairing list my-channel

# Approve a request by code
zero channel pairing approve my-channel <CODE>
```
...
The bot runs in the foreground. Press `Ctrl+C` to stop, or use `zero channel stop` from another terminal.

### Multi-Channel Mode

When you run `zero channel start` without a name, all channels defined in `settings.json` start together sharing a single agent process. Each channel maintains its own sessions — a Telegram user and a WeChat user get separate conversations, even though they share the same agent.
...
- **Duplicate prevention**: Running `zero channel start` while a service is already running will show an error instead of starting a second instance
- **`zero channel stop`**: Gracefully stops the running service from another terminal
- **`zero channel status`**: Shows whether the service is running, its uptime, and session counts per channel
...
- On clean shutdown (Ctrl+C or `zero channel stop`): session data is cleared — the next start is always fresh
