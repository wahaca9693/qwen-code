...
Subagents are specialized AI assistants that handle specific types of tasks within ZERO Agent. They allow you to delegate focused work to AI agents that are configured with task-specific prompts, tools, and behaviors.
...
In addition to named subagents, ZERO Agent supports **implicit forking** — when the AI omits the `subagent_type` parameter, it triggers a fork that inherits the parent's full conversation context.
...
ZERO Agent proactively delegates tasks based on:
