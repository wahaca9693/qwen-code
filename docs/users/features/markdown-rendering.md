...
ZERO Agent renders common Markdown structures directly in the TUI so model
...
To start ZERO Agent in raw mode by default, set `ui.renderMode`:
...
1. If enabled and supported, ZERO Agent asks Mermaid CLI (`mmdc`) to render the
...
3. Otherwise, ZERO Agent falls back to a terminal wireframe or compact text
   preview.
4. If a Mermaid diagram type cannot be previewed, ZERO Agent shows the original
...
```bash
ZERO_CODE_MERMAID_IMAGE_RENDERING=1 qwen
```
...
| ------------------------------------------- | ----------------------------------------------------------------------------------- |
| `ZERO_CODE_MERMAID_IMAGE_RENDERING=1`       | Enables external Mermaid image rendering.                                           |
| `ZERO_CODE_DISABLE_MERMAID_IMAGES=1`        | Disables Mermaid image rendering even when enabled elsewhere.                       |
| `ZERO_CODE_MERMAID_IMAGE_PROTOCOL=kitty`    | Forces Kitty protocol output. Useful for terminals such as Kitty and Ghostty.       |
| `ZERO_CODE_MERMAID_IMAGE_PROTOCOL=iterm2`   | Requests iTerm2 inline images. Interactive TUI rendering falls back to text/ANSI.   |
| `ZERO_CODE_MERMAID_IMAGE_PROTOCOL=off`      | Disables terminal image protocols and allows text or `chafa` fallback.              |
| `ZERO_CODE_MERMAID_MMD_CLI=/path/to/mmdc`   | Uses a specific Mermaid CLI executable.                                             |
| `ZERO_CODE_MERMAID_ALLOW_NPX=1`             | Allows ZERO Agent to run `npx @mermaid-js/mermaid-cli` when `mmdc` is not installed. |
| `ZERO_CODE_MERMAID_ALLOW_LOCAL_RENDERERS=1` | Allows project-local renderer binaries under `node_modules/.bin`.                   |
| `ZERO_CODE_MERMAID_RENDER_WIDTH=1200`       | Overrides the PNG render width.                                                     |
| `ZERO_CODE_MERMAID_RENDER_TIMEOUT_MS=10000` | Overrides the external render timeout, capped at 60000 ms.                          |
| `ZERO_CODE_MERMAID_CELL_ASPECT_RATIO=0.5`   | Adjusts image row fitting for terminal font cell geometry.                          |

The first image render can be slow, especially when `npx` needs to resolve or
download Mermaid CLI. During streaming, ZERO Agent shows a bounded text preview
...
ZERO Agent supports basic inline and block LaTeX rendering in the terminal:
