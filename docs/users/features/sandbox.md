...
This document explains how to run ZERO Agent inside a sandbox to reduce risk when tools execute shell commands or modify files.

## Prerequisites

Before using sandboxing, you need to install and set up ZERO Agent:
...
> [!note]
>
> **Naming note:** Some sandbox-related environment variables may have used the `GEMINI_*` prefix historically. All new environment variables use the `ZERO_*` prefix.
...
By default, ZERO Agent uses a published sandbox image (configured in the CLI package) and will pull it as needed.
...
zero -s -p "analyze the code structure"

# Or enable sandboxing for your shell session (recommended for CI / scripts)
export ZERO_SANDBOX=true   # true auto-picks a provider (see notes below)
zero -p "run the test suite"
...
>
> - On **macOS**, `ZERO_SANDBOX=true` typically selects `sandbox-exec` (Seatbelt) if available.
> - On **Linux/Windows**, `ZERO_SANDBOX=true` requires `docker` or `podman` to be installed.
> - To force a provider, set `ZERO_SANDBOX=docker|podman|sandbox-exec`.

## Configuration

### Enable sandboxing (in order of precedence)

1. **Environment variable**: `ZERO_SANDBOX=true|false|docker|podman|sandbox-exec`
...
> [!important]
>
> If `ZERO_SANDBOX` is set, it **overrides** the CLI flag and `settings.json`.

### Configure the sandbox image (Docker/Podman)

- **CLI flag**: `--sandbox-image <image>`
- **Environment variable**: `ZERO_SANDBOX_IMAGE=<image>`
- **Settings file**: `tools.sandboxImage` in your `settings.json` (e.g., `{"tools": {"sandboxImage": "ghcr.io/qwenlm/zero-agent:0.14.1"}}`)

Priority order (highest to lowest):

1. `--sandbox-image`
2. `ZERO_SANDBOX_IMAGE`
3. `tools.sandboxImage`
4. Built-in default image from the CLI package (for example `ghcr.io/qwenlm/zero-agent:<version>`)

`settings.env.ZERO_SANDBOX_IMAGE` also works as a generic env injection mechanism, but `tools.sandboxImage` is the preferred persistent setting.
...
- Set `ZERO_SANDBOX_PROXY_COMMAND=<command>`
...
On Linux, ZERO Agent defaults to enabling UID/GID mapping so the sandbox runs as your user (and reuses the mounted `~/.qwen`). Override with:
...
```bash
ZERO_SANDBOX=docker BUILD_SANDBOX=1 zero -s
```
...
```bash
DEBUG=1 zero -s -p "debug command"
```

**Note:** If you have `DEBUG=true` in a project's `.env` file, it won't affect the CLI due to automatic exclusion. Use `.qwen/.env` files for ZERO Agent-specific debug settings.

### Inspect sandbox

```bash
# Check environment
zero -s -p "run shell command: env | grep SANDBOX"

# List mounts
zero -s -p "run shell command: mount | grep workspace"
```
