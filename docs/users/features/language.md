...
ZERO Agent is built for multilingual workflows: it supports UI localization (i18n/l10n) in the CLI, lets you choose the assistant output language, and allows custom UI language packs.

## Overview

From a user point of view, ZERO Agent’s “internationalization” spans multiple layers:
...
On first startup, ZERO Agent detects your system locale and sets the UI language automatically.

Detection priority:

1. `ZERO_CODE_LANG` environment variable
...
On first startup, if no `output-language.md` file exists, ZERO Agent automatically creates one based on your system locale. For example:
...
> [!note]
>
> After changing the output language, restart ZERO Agent for the change to take effect.
...
> For a concrete example, see [PR #1238: feat(i18n): add Russian language support](https://github.com/ZEROLM/zero-agent/pull/1238).
