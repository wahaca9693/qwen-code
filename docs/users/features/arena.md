...
1. **Worktree setup**: ZERO Agent creates isolated Git worktrees for each agent at `~/.qwen/arena/<session-id>/worktrees/<model-name>/`. Each worktree mirrors your current working directory state exactly — including staged changes, unstaged changes, and untracked files.
...
Agent Arena is one of several planned multi-agent modes in ZERO Agent. **Agent Team** and **Agent Swarm** are not yet implemented — the table below describes their intended design for reference.
...
- **Manual parallel sessions**: Run multiple ZERO Agent sessions yourself in separate terminals with [Git worktrees](https://git-scm.com/docs/git-worktree) for full manual control
