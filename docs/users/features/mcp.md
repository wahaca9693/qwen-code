# Connect ZERO Agent to tools via MCP

ZERO Agent can connect to external tools and data sources through the [Model Context Protocol (MCP)](https://modelcontextprotocol.io/introduction). MCP servers give ZERO Agent access to your tools, databases, and APIs.

## What you can do with MCP

With MCP servers connected, you can ask ZERO Agent to:
...
ZERO Agent loads MCP servers from `mcpServers` in your `settings.json`. You can configure servers either:

- By editing `settings.json` directly
- By using `zero mcp` commands (see [CLI reference](#qwen-mcp-cli))
...
```bash
zero mcp add --transport http my-server http://localhost:3000/mcp
```
...
3. Restart ZERO Agent in the same project (or start it if it wasn’t running yet), then ask the model to use tools from that server.
...
```bash
zero mcp add --scope user --transport http my-server http://localhost:3000/mcp
```
...
### Configure via `settings.json` vs `zero mcp add`
...
```bash
zero mcp add pythonTools -e DATABASE_URL=$DB_CONNECTION_STRING -e API_KEY=$EXTERNAL_API_KEY \
...
```bash
zero mcp add --transport http httpServerWithAuth http://localhost:3000/mcp \
...
```bash
zero mcp add --transport sse sseServer http://localhost:8080/sse --timeout 30000
```
...
ZERO Agent supports OAuth 2.0 authentication for MCP servers. This is useful when accessing remote servers that require authentication.

#### Basic usage

When you add an MCP server with OAuth credentials, ZERO Agent will automatically handle the authentication flow:
...
- **Local development**: By default, ZERO Agent uses `http://localhost:7777/oauth/callback`. This works when running ZERO Agent on your local machine with a local browser.

- **Remote/cloud deployments**: When running ZERO Agent on remote servers, cloud IDEs, or web terminals, the default `localhost` redirect will NOT work. You MUST configure `--oauth-redirect-uri` to point to a publicly accessible URL that can receive the OAuth callback.
...
Use the `/mcp auth` command within ZERO Agent to manage OAuth authentication interactively.

### Tool filtering (allow/deny tools per server)

Use `includeTools` / `excludeTools` to restrict tools exposed by a server (from ZERO Agent’s perspective).
...
- **Server shows “Disconnected” in `zero mcp list`**: verify the URL/command is correct, then increase `timeout`.
- **Stdio server fails to start**: use an absolute `command` path, and double-check `cwd`/`env`.
- **Environment variables in JSON don’t resolve**: ensure they exist in the environment where ZERO Agent runs (shell vs GUI app environments can differ).
...
### Manage MCP servers with `zero mcp`
