...
ZERO Agent can generate a short, git-commit-subject-style label after each tool batch completes, summarizing what the batch accomplished. The label appears inline in the transcript and replaces the generic `Tool × N` header in compact mode.
...
After a tool batch finalizes, ZERO Agent fires a fire-and-forget call to the configured fast model with:
...
`ZERO_CODE_EMIT_TOOL_USE_SUMMARIES` overrides the `experimental.emitToolUseSummaries` setting for the current session:

- `ZERO_CODE_EMIT_TOOL_USE_SUMMARIES=0` or `=false` — force off.
- `ZERO_CODE_EMIT_TOOL_USE_SUMMARIES=1` or `=true` — force on.
...
- Disable the feature entirely with `experimental.emitToolUseSummaries: false` (or `ZERO_CODE_EMIT_TOOL_USE_SUMMARIES=0`).
...
If you do not want the extra cost, turn the feature off via `experimental.emitToolUseSummaries: false` or `ZERO_CODE_EMIT_TOOL_USE_SUMMARIES=0`.
