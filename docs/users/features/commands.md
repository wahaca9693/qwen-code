...
This document details all commands supported by ZERO Agent, helping you efficiently manage sessions, customize the interface, and control its behavior.

ZERO Agent commands are triggered through specific prefixes and fall into three categories:

| Prefix Type                | Function Description                                | Typical Use Case                                                 |
| -------------------------- | --------------------------------------------------- | ---------------------------------------------------------------- |
| Slash Commands (`/`)       | Meta-level control of ZERO Agent itself              | Managing sessions, modifying settings, getting help              |
...
Slash commands are used to manage ZERO Agent sessions, interface, and basic behavior.
...
| `/qc-helper` | Answer questions about ZERO Agent usage and configuration            | `/qc-helper how do I configure MCP?`              |
...
| `/bug`      | Submit issue about ZERO Agent                    | `/bug Button click unresponsive` |
...
Use `/auth` inside a ZERO Agent session to configure authentication. Use `/doctor` to inspect the current authentication and environment status.
...
> [!note]
>
> The standalone `zero auth` CLI command has been removed. Legacy invocations such as `zero auth status` print a removal notice with migration guidance. See the [Authentication](../configuration/auth) page for full details.
...
Exclamation commands allow you to execute system commands directly within ZERO Agent.
...
Environment Variables: Commands executed via `!` will set the `ZERO_CODE=1` environment variable.
